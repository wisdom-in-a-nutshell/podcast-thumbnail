...
2) For each speaker, choose {timestamps_per_speaker} timestamps (seconds, ascending) that give the best headshot candidates:
...
4) Mark likely role if obvious: host | guest | unknown.
5) Return confidence 0–1 for each speaker grouping.
6) Do NOT include images—only metadata.
...
{{
  "speakers": [
    {{
      "id": "speaker_1",
      "timestamps_s": [12.3, 55.8, 130.4, 245.1],
...
If two clusters seem to be the same person, merge and pick the best {timestamps_per_speaker} timestamps overall.
"""
